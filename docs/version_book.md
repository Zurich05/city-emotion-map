...
已完成：
- 新增通用 `require_roles` 权限依赖。
- 数据写操作接口限制为 `admin` / `analyst`。
- `viewer` 角色只能访问只读接口，不能执行导入、采集、清洗、情感分析和备份恢复。
- 用户管理接口继续仅 `admin` 可访问。
- 前端登录后保存角色信息。
- 前端按角色隐藏“用户权限”菜单。
- 前端路由拦截非管理员访问用户管理页。
- 新增后端测试覆盖 viewer 禁止写操作、analyst 可执行数据任务但不可管理用户。
- 本阶段变更已完成测试、提交并推送到 GitHub。

未实现：
- 暂未实现按钮级权限提示，非授权操作主要依赖菜单隐藏和后端 403。
- 暂未实现动态权限配置表，角色权限仍在代码中定义。
